        ...
        def create_angle(tracker, origin, phi = False):
            ...
            if phi == False:
                line = always_redraw(lambda : Line(
                    start=origin,
                    end=origin + 3*np.array([np.cos(tracker.get_value()-PI/2), -np.sin(tracker.get_value()-PI/2),0]),
                    color=WHITE
                ))
                ...
                label = always_redraw(lambda : MathTex("\\theta = %d^{{o}}" % (180 / PI * tracker.get_value()), color=WHITE, font_size=56).next_to(arc.get_end()+RIGHT))

                return VGroup(line, arc, label)
            
            else:
                line = always_redraw(lambda : Line(
                    start=origin,
                    end=origin + 3*np.array([np.cos(tracker.get_value()), np.sin(tracker.get_value()), 0]),
                    color=ORANGE
                ))
                ...
                label = always_redraw(lambda: MathTex("\\phi = %d^{{o}}" % (180 / PI * tracker.get_value()), color=WHITE, font_size=56).next_to(arc.get_end()+RIGHT))
