...
class Sphere3D(ThreeDScene):
    def construct (self):
        ...
        inte2 = MathTex(r"V_{\text{sphere}} = \int_0^R r^2dr \int_0^\pi \sin(\theta)d\theta \int_0^{2\pi} d\phi").move_to(final_eq_pos)
        inte3 = MathTex(r"V_{\text{sphere}} = \left[\frac{r^3}{3}\right]_0^R \left[-\cos(\theta)\right]_0^{\pi} \left[\phi\right]_0^{2\pi}").move_to(final_eq_pos)
        inte4 = MathTex(r"V_{\text{sphere}} = \frac{R^3}{3} \cdot \left( (-\cos(\pi)) - (-\cos(0)) \right) \cdot 2\pi").move_to(final_eq_pos) # More explicit
        inte4_simplified = MathTex(r"V_{\text{sphere}} = \frac{R^3}{3} \cdot (1 - (-1)) \cdot 2\pi = \frac{R^3}{3} \cdot 2 \cdot 2\pi").move_to(final_eq_pos)
        inte5 = MathTex(r"V_{\text{sphere}} = \frac{4\pi}{3}R^3").move_to(final_eq_pos)
        ...
        self.wait(1)  # Longer wait for complex step
        self.play(Transform(displ, inte4))
        self.wait(1)  # Longer wait
        self.play(Transform(displ, inte4_simplified))
        self.wait(1)
