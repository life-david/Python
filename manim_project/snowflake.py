...
def koch_curve(points, order):
    ...
    for i in range(len(points) - 1):
        ...
        p2 = points[i+1]
        delta = p2 - p1
        # Xác định các điểm chia đoạn theo tỉ lệ 1/3 và 2/3
        a = p1 + delta / 3
        b = p1 + 2 * delta / 3
        # Tính điểm "đỉnh" của tam giác ngoại tiếp bằng cách xoay đoạn a->b 60 độ
        angle = np.pi / 3  # 60 độ
        ...
        peak = p1 + np.array([rotated_vector[0], rotated_vector[1], 0])
...
def koch_snowflake(order):
    """
    Tạo dãy điểm của bông tuyết Koch bằng cách áp dụng quy tắc cho từng cạnh của tam giác.
    """
...
class KochSnowflakeAnimation(Scene):
    def construct(self):
        # Danh sách các mức độ (order) của bông tuyết
        orders = [0, 1, 2, 3, 4]
        # Bắt đầu với tam giác (order 0)
